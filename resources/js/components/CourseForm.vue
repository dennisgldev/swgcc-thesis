        ...
        if (index === this.course.sections.length - 1) {
          ...
          this.$nextTick(() => {
            this.lessonEditor = new Editor({
              extensions: [StarterKit, TextAlign, Underline],
              content: section.lesson.content || '', // Inicia con el contenido si existe
            });
          });
        } else {
          section.lesson = null;
    ...
    addQuestion(sectionIndex) {
      ...
      if (section.lesson) {
        section.lesson.questions.push({
          text: '',
          type: 'única', // Valor esperado por el backend
          points: 1,
          answers: [],
        });
      }
    ...
    prepareFormData() {
      ...
      this.course.sections.forEach((section, sectionIndex) => {
        ...
        if (section.lesson) {
          ...
          formData.append(`sections[${sectionIndex}][lesson][content]`, this.lessonEditor.getHTML());
